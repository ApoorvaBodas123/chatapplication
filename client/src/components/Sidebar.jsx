    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
      <div className="pb-5">
        <div className="flex justify-between items-center">
          <img src={assets.logo_icon} alt="logo" className="max-w-15" />
             ...
             <div className="absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#032130] border border-gray-600 text-gray-100 hidden group-hover:block">
      ...
      <div className="flex flex-col">
          {userDummyData.map((user,index)=>(
             <div onClick={()=>{setSelectedUser(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#032130]'}`}>
