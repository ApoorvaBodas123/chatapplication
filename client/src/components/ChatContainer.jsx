  ...
  return selectedUser ?(
    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
       <div className='flex text-white items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <p className='flex-1 text-lg flex items-center gap-2'>Martin Jhonson
     ...
     <div className='flex flex-col h-[calc(100%-120px)] overflow-scroll p-3 pb-6'>
       {messagesDummyData.map((msg,index)=>(
        <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
           ...
           <p className={`p-2 max-w-[200px] md:text-sm font-light bg-blue-300 text-gray rounded-lg mb-10 break-all ${msg.senderId === '680f50e4f10f3cd28382ecf9' ? 'rounded-br-none' : 'rounded-bl-none'}`}>
           {msg.text}
           </p>
           )}
     ...
     {/* ------ bottom */}
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3">
       <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
        <input type="text" placeholder="Send a Message" className='flex-1 text-sm text-white p-3 border-none rounded-lg outline-none placeholder-gray-400'/>
